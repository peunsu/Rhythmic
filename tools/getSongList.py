...
def getList(input_df, loc, game):
    try:
        ...
        for i, j in zip(range(0, endLoc-startLoc), range(startLoc, endLoc)):
            if game == 0:
                arr.append(str(j) + ". " + df_temp.iloc[i, 1] + " - " + df_temp.iloc[i, 2])
                output = "\n".join(arr)
                ...
                arr.append(str(j) + ". " + df_temp.iloc[i, 0] + " - " + df_temp.iloc[i, 1])
                output = "\n".join(arr)
                ...
                arr.append(str(j) + ". " + df_temp.iloc[i, 0] + " - " + df_temp.iloc[i, 1])
                output = "\n".join(arr)
                output_text = "```css\n[VOEZ Song List (" + str(firstPage) + " of " + str(endPage) + ")]\n\n" + output + "\n```"
            elif game == 6:
                arr.append(str(j) + ". " + df_temp.iloc[i, 0] + " - " + df_temp.iloc[i, 1])
                output = "\n".join(arr)
                output_text = "```css\n[Phigros Song List (" + str(firstPage) + " of " + str(endPage) + ")]\n\n" + output + "\n```"
